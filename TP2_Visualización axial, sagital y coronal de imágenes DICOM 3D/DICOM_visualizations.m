...
path = "DICOM\"; %Ruta relativa de cualquier imágen DICOM seleccionada
...
[x, n_img] = size(names); % Se obtiene el tamaño de los componentes de 'names', 
...
for i = 1:1:n_img % Para todas las imágenes de la carpeta:
    ...
    current_max = max(max(current_data)); % Obtiene los valores máximos y mínimos 
    % dentro de la información de la imágen
    current_min = min(min(current_data)); 
    DCM = dicominfo(h); %Obtiene la metadata de la imágen
    Image(:,:,DCM.InstanceNumber) = current_data; % Se acomoda dentro de la matriz 
...
figure(1);
B=imresize3(Image, [round(n_rows) round(n_cols) round(n_img*1.22)],"AntiAliasing",false);
% Cambia el tamaño de la imágen para que se vea más proporcional.
for i =1:1:n_img % Por cada una de las imágenes de la carpeta
    disp(i)
    slice = permute(B(:,i,:),[3 1 2]); % i:: 321 // ::i 123 // :i:312  -- 
    % diferentes tipos de cortes de la imágen
    imshow(slice(:,:,1),[global_min, global_max]) %(rescalada) los atributos de mínimo y
